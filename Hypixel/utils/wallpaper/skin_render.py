...
async def render_3d_head(
    ...
    im = await Render(user, vr, hr, hrh, 0, 0, 0, 0, ratio, True, display_hair, aa).get_render(skin_image)
    return im
...
class Render:
    ...
    async def get_render(self, skin):
        ...
        self.determine_format(hd_ratio, skin)
        self.calculate_angles()
        self.determine_faces()
        self.generate_polygons(hd_ratio, self.arm_width, skin)
    ...
    def determine_format(self, hd_ratio, skin):
        for i in range(0, 13 * hd_ratio):
            if skin.getpixel((54 * hd_ratio, 20 * hd_ratio + i))[3] == 0:
                self.arm_width = 3
                break
        if self.arm_width == 4:
            for i in range(0, 13 * hd_ratio):
                if skin.getpixel((46 * hd_ratio, 52 * hd_ratio + i))[3] == 0:
                    self.arm_width = 3
                    break
    ...
    def generate_polygons(self, hd_ratio, arm_width, skin):
        self.polygons = {
        ...
        if self.head_only == False:
            ...
            """Right arm"""
            volume_points = {}
            for i in range(0, (arm_width + 1) * hd_ratio):
                for j in range(0, 13 * hd_ratio):
                    volume_points = append_dict(volume_points, i, j, 0, Point({"x": i - arm_width * hd_ratio, "y": j + 8 * hd_ratio, "z": 0}))
                    volume_points = append_dict(volume_points, i, j, 4 * hd_ratio, Point({"x": i - arm_width * hd_ratio, "y": j + 8 * hd_ratio, "z": 4 * hd_ratio}))

            for j in range(0, 13 * hd_ratio):
                for k in range(0, 5 * hd_ratio):
                    volume_points = append_dict(volume_points, 0, j, k, Point({"x": -arm_width * hd_ratio, "y": j + 8 * hd_ratio, "z": k}))
                    volume_points = append_dict(volume_points, 4 * hd_ratio, j, k, Point({"x": 0, "y": j + 8 * hd_ratio, "z": k}))

            for i in range(0, (arm_width + 1) * hd_ratio):
                for k in range(0, 5 * hd_ratio):
                    volume_points = append_dict(volume_points, i, 0, k, Point({"x": i - arm_width * hd_ratio, "y": 8 * hd_ratio, "z": k}))
                    volume_points = append_dict(volume_points, i, 12 * hd_ratio, k, Point({"x": i - arm_width * hd_ratio, "y": 20 * hd_ratio, "z": k}))

            if "back" in self.visible_faces["r_arm"]["front"]:
                for i in range(0, arm_width * hd_ratio):
                    for j in range(0, 12 * hd_ratio):
                        if arm_width == 3:
                            pixel = ((54 * hd_ratio - 1) - i, 20 * hd_ratio + j)
                        else:
                            pixel = ((56 * hd_ratio - 1) - i, 20 * hd_ratio + j)
                        color = skin.getpixel(pixel)
                        if color[3] != 0:
            ...
            if "left" in self.visible_faces["r_arm"]["front"]:
                for j in range(0, 12 * hd_ratio):
                    for k in range(0, 4 * hd_ratio):
                        if arm_width == 3:
                            pixel = ((51 * hd_ratio - 1) - k, 20 * hd_ratio + j)
                        else:
                            pixel = ((52 * hd_ratio - 1) - k, 20 * hd_ratio + j)
                        color = skin.getpixel(pixel)
                        if color[3] != 0:
            ...
            if "bottom" in self.visible_faces["r_arm"]["front"]:
                for i in range(0, arm_width * hd_ratio):
                    for k in range(0, 4 * hd_ratio):
                        if arm_width == 3:
                            pixel = (47 * hd_ratio + i, 16 * hd_ratio + k)
                        else:
                            pixel = (48 * hd_ratio + i, 16 * hd_ratio + k)
                        color = skin.getpixel(pixel)
                        if color[3] != 0:
            ...
            if self.layers:
                ...
                for i in range(0, (arm_width + 1) * hd_ratio):
                    for j in range(0, 13 * hd_ratio):
                        volume_points = append_dict(volume_points, i, j, 0, Point({"x": (i * (arm_width + 0.25) / arm_width - 0.125 * hd_ratio) - arm_width * hd_ratio, "y": (j * 12.25 / 12 - 0.125 * hd_ratio) + 8 * hd_ratio, "z": -0.125 * hd_ratio}))
                        volume_points = append_dict(volume_points, i, j, 4 * hd_ratio, Point({"x": (i * (arm_width + 0.25) / arm_width - 0.125 * hd_ratio) - arm_width * hd_ratio, "y": (j * 12.25 / 12 - 0.125 * hd_ratio) + 8 * hd_ratio, "z": 4.125 * hd_ratio}))

                for j in range(0, 13 * hd_ratio):
                    for k in range(0, 5 * hd_ratio):
                        volume_points = append_dict(volume_points, 0, j, k, Point({"x": -(arm_width + 0.125) * hd_ratio, "y": (j * 12.25 / 12 - 0.125 * hd_ratio) + 8 * hd_ratio, "z": k * 4.25 / 4 - 0.125 * hd_ratio}))
                ...
                for i in range(0, (arm_width + 1) * hd_ratio):
                    for k in range(0, 5 * hd_ratio):
                        volume_points = append_dict(volume_points, i, 0, k, Point({"x": (i * (arm_width + 0.25) / arm_width - 0.125 * hd_ratio) - arm_width * hd_ratio, "y": 7.875 * hd_ratio, "z": k * 4.25 / 4 - 0.125 * hd_ratio}))
                        volume_points = append_dict(volume_points, i, 12 * hd_ratio, k, Point({"x": (i * (arm_width + 0.25) / arm_width - 0.125 * hd_ratio) - arm_width * hd_ratio, "y": 20.125 * hd_ratio, "z": k * 4.25 / 4 - 0.125 * hd_ratio}))

                for i in range(0, arm_width * hd_ratio):
                    for j in range(0, 12 * hd_ratio):
                        if arm_width == 3:
                            pixel = ((54 * hd_ratio - 1) - i, 36 * hd_ratio + j)
                        else:
                            pixel = ((56 * hd_ratio - 1) - i, 36 * hd_ratio + j)
                        color = skin.getpixel(pixel)
                        if color[3] != 0:
                            self.polygons["r_arm_layer"]["back"].append(Polygon([
                                ...
                                volume_points[i][j + 1][0]],
                                color))
                        color = skin.getpixel((44 * hd_ratio + i, 36 * hd_ratio + j))
                        if color[3] != 0:
                ...
                for j in range(0, 12 * hd_ratio):
                    for k in range(0, 4 * hd_ratio):
                        color = skin.getpixel((40 * hd_ratio + k, 36 * hd_ratio + j))
                        if color[3] != 0:
                            self.polygons["r_arm_layer"]["right"].append(Polygon([
                                ...
                                color))
                        if arm_width == 3:
                            pixel = ((51 * hd_ratio - 1) - k, 36 * hd_ratio + j)
                        else:
                            pixel = ((52 * hd_ratio - 1) - k, 36 * hd_ratio + j)
                        color = skin.getpixel(pixel)
                        if color[3] != 0:
                ...
                for i in range(0, arm_width * hd_ratio):
                    for k in range(0, 4 * hd_ratio):
                        color = skin.getpixel((44 * hd_ratio + i, 32 * hd_ratio + k))
                        if color[3] != 0:
                            self.polygons["r_arm_layer"]["top"].append(Polygon([
                                ...
                                color))
                        if arm_width == 3:
                            pixel = (47 * hd_ratio + i,32 * hd_ratio + k)
                        else:
                            pixel = (48 * hd_ratio + i,32 * hd_ratio + k)
                        color = skin.getpixel(pixel)
                        if color[3] != 0:
            ...
            for j in range(0, 13 * hd_ratio):
                for k in range(0, 5 * hd_ratio):
                    ...
                    volume_points = append_dict(volume_points, 4 * hd_ratio, j, k, Point({"x": (8 + arm_width) * hd_ratio, "y": j + 8 * hd_ratio, "z": k}))
            ...
            if "back" in self.visible_faces["l_arm"]["front"]:
                for i in range(0, arm_width * hd_ratio):
                    for j in range(0, 12 * hd_ratio):
                        if arm_width == 3:
                            pixel = (46 * hd_ratio - 1 - i, 52 * hd_ratio + j)
                        else:
                            pixel = (48 * hd_ratio - 1 - i, 52 * hd_ratio + j)
                        color = skin.getpixel(pixel)
                        if color[3] != 0:
            ...
            if "left" in self.visible_faces["l_arm"]["front"]:
                for j in range(0, 12 * hd_ratio):
                    for k in range(0, 4 * hd_ratio):
                        if arm_width == 3:
                            pixel = (43 * hd_ratio - 1 - k, 52 * hd_ratio + j)
                        else:
                            pixel = (44 * hd_ratio - 1 - k, 52 * hd_ratio + j)
                        color = skin.getpixel(pixel)
                        if color[3] != 0:
            ...
            if "bottom" in self.visible_faces["l_arm"]["front"]:
                for i in range(0, arm_width * hd_ratio):
                    for k in range(0, 4 * hd_ratio):
                        if arm_width == 3:
                            pixel = (39 * hd_ratio + i, 48 * hd_ratio + k)
                        else:
                            pixel = (40 * hd_ratio + i, 48 * hd_ratio + k)
                        color = skin.getpixel(pixel)
                        if color[3] != 0:
            ...
            if self.layers:
                ...
                for i in range(0, (arm_width + 1) * hd_ratio):
                    for j in range(0, 13 * hd_ratio):
                        volume_points = append_dict(volume_points, i, j, 0, Point({"x": (i * (arm_width + 0.25) / arm_width - 0.125 * hd_ratio) + 8 * hd_ratio, "y": (j * 12.25 / 12 - 0.125 * hd_ratio) + 8 * hd_ratio, "z": -0.125 * hd_ratio}))
                        volume_points = append_dict(volume_points, i, j, 4 * hd_ratio, Point({"x": (i * (arm_width + 0.25) / arm_width - 0.125 * hd_ratio) + 8 * hd_ratio, "y": (j * 12.25 / 12 - 0.125 * hd_ratio) + 8 * hd_ratio, "z": 4.125 * hd_ratio}))
                ...
                for j in range(0, 13 * hd_ratio):
                    for k in range(0, 5 * hd_ratio):
                        ...
                        volume_points = append_dict(volume_points, 4 * hd_ratio, j, k, Point({"x": (arm_width + 8.125) * hd_ratio, "y": (j * 12.25 / 12 - 0.125 * hd_ratio) + 8 * hd_ratio, "z": k * 4.25 / 4 - 0.125 * hd_ratio}))

                for i in range(0, (arm_width + 1) * hd_ratio):
                    for k in range(0, 5 * hd_ratio):
                        volume_points = append_dict(volume_points, i, 0, k, Point({"x": (i * (arm_width + 0.25) / arm_width - 0.125 * hd_ratio) + 8 * hd_ratio, "y": 7.875 * hd_ratio, "z": k * 4.25 / 4 - 0.125 * hd_ratio}))
                        volume_points = append_dict(volume_points, i, 12 * hd_ratio, k, Point({"x": (i * (arm_width + 0.25) / arm_width - 0.125 * hd_ratio) + 8 * hd_ratio, "y": 20.125 * hd_ratio, "z": k * 4.25 / 4 - 0.125 * hd_ratio}))

                for i in range(0, arm_width * hd_ratio):
                    for j in range(0, 12 * hd_ratio):
                        if arm_width == 3:
                            pixel = (62 * hd_ratio - 1 - i, 52 * hd_ratio + j)
                        else:
                            pixel = (64 * hd_ratio - 1 - i, 52 * hd_ratio + j)
                        color = skin.getpixel(pixel)
                        if color[3] != 0:
                    ...
                    for k in range(0, 4 * hd_ratio):
                        ...
                        if color[3] != 0:
                            self.polygons["l_arm_layer"]["right"].append(Polygon([
                                ...
                                color))
                        if arm_width == 3:
                            pixel = (59 * hd_ratio - 1 - k, 52 * hd_ratio + j)
                        else:
                            pixel = (60 * hd_ratio - 1 - k, 52 * hd_ratio + j)
                        color = skin.getpixel(pixel)
                        if color[3] != 0:
                    ...
                    for k in range(0, 4 * hd_ratio):
                        ...
                        if color[3] != 0:
                            self.polygons["l_arm_layer"]["top"].append(Polygon([
                                ...
                                color))
                        if arm_width == 3:
                            pixel = (55 * hd_ratio + i, 48 * hd_ratio + k)
                        else:
                            pixel = (56 * hd_ratio + i, 48 * hd_ratio + k)
                        color = skin.getpixel(pixel)
                        if color[3] != 0:
...
if __name__ == "__main__":
    ...
    user = "sucr_mausi"
    render = loop.run_until_complete(render_3d_skin(user, pos[0], pos[1], pos[2], pos[3], pos[4], pos[5], pos[6], ratio=50, display_hair=True, aa=False, display_second_layer=True, skin_image=None))
    render.show()